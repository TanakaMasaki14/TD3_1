...
void NoviceSystem::CreateConstBuffer() {
	...
	//平行投影による射影行列の生成
	Matrix4 mat = MathUtility::Matrix4Orthographic(
	  0, float(dxCommon_->GetBackBufferWidth()), float(dxCommon_->GetBackBufferHeight()), 0, 0, 1);
...
void NoviceSystem::DrawTriangles(std::span<VertexPosColor> trianglePoints) {
	...
	auto numTriangles = trianglePoints.size() / 3;
...
void NoviceSystem::DrawLines(std::span<VertexPosColor> linePoints) {
	...
	auto numLines = linePoints.size() / 2;
...
void NoviceSystem::DrawQuad(
  ...
  int srcH, int textureHandle, unsigned int color) {
	...
	// 平行投影による射影行列の設定
	constBufferForQuads_[indexQuad_].address->mat = MathUtility::Matrix4Orthographic(
	  0, float(dxCommon_->GetBackBufferWidth()), float(dxCommon_->GetBackBufferHeight()), 0, 0, 1);
	// 頂点バッファの設定
...
void NoviceSystem::ScreenPrintf(int x, int y, const char* text, va_list& args) {
	...
	// 書式付き文字列を変換
	int w = vsnprintf(textBuffer.data(), textBuffer.size() - 1, text, args);
...
void Novice::Initialize(const char* title, int width, int height) {
	...
	sWinApp->CreateGameWindow(
	  title, WS_OVERLAPPEDWINDOW & ~(WS_MAXIMIZEBOX | WS_THICKFRAME), width, height);
		...
		for (size_t index = 0; index < kNumDivision; ++index) {
			...
			if (index == 0) {
				...
				points[index * 3 + 1].pos = {
				  baseX * angleCos - baseY * angleSin + x, baseY * angleCos + baseX * angleSin + y,
				  0};
			} else {
...
int Novice::IsTriggerButton(int stickNo, PadButton button) {
	...
	if (
	  sNoviceSystem->GetJoystickState(stickNo, stateDInput) &&
